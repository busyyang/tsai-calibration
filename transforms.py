...
def projectPoints(points, f):
    ...
    return list(map(projectPoint, points))


# sensor coordinates (2d, in mm) -> normalised image coordinates
def sensorToNormal(points, pixelSize, resolution):
    s2n = np.array([[2.0 / (pixelSize[0] * resolution[0]), 0.0, 0.0], [0.0, 2.0 / (pixelSize[1] * resolution[1]), 0.0],
...
def normalToPixel(points, resolution):
    n2p = np.array([[-resolution[0] / 2.0, 0.0, resolution[0] / 2.0], [0.0, -resolution[1] / 2.0, resolution[1] / 2.0],
                    [0.0, 0.0, 1.0]], np.float64)
...
def pixelToNormal(points, resolution):
    ...
    def transform(point):
        ...
        # =  [ xd, yd, 1.0 ]  transform pixel coordinates to sensor coordinates (in mm, origin at center of camera)
        return point._replace(normal=np.dot(p2n, p))
...
def normalToSensor(points, resolution, pixelSize):
    ...
    def transform(point):
        ...
        # =  [ xd, yd, 1.0 ]  transform pixel coordinates to sensor coordinates (in mm, origin at center of camera)
        return point._replace(sensor=np.dot(n2s, p))
...
def worldToSensor(points, params, pixelSize, resolution, yOffset, kappa=0.0):
    return sensorToPixel(projectPoints(worldToCamera(points, params, yOffset), params['f']), pixelSize, resolution,
                         kappa)
